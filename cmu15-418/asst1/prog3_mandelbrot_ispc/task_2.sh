...
for v in "${V_VALUES[@]}"; do
  ...
  # 重命名生成的 PNG 文件
  mv mandelbrot-serial.png "mandelbrot-serial-v$v--task.png"
  mv mandelbrot-ispc.png "mandelbrot-ispc-v$v--task.png"
  echo "Saved mandelbrot-serial-v$v--task.png."
  echo "----------------------------"
done
